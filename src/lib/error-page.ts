export function renderErrorPage(err?: unknown): string {
  const errorText = err ? `<pre style="text-align:left;background:#eee;padding:10px;border-radius:6px;font-size:12px;overflow:auto;max-height:200px;">${String(err)}</pre>` : "";
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
